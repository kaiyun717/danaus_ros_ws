...
class ETHTrackingNode:
    ...
    def ned2enu(self, xyz):
        return np.array([xyz[0], -xyz[1], -xyz[2]])

    def enu2ned(self, xyz):
        return np.array([xyz[0], -xyz[1], -xyz[2]])
    ...
    def _quad_lqr_controller(self):
        ...
        quad_xyz_ang_vel_body_enu = self.quad_cb.get_xyz_angular_velocity_body()
        if self.mode == "sim":
        ...
        # Set P = 1, I = 0, D = 0
        # u[:3] = rate_sp - rate 
        quad_xyz_ang_vel_body_ned = self.enu2ned(quad_xyz_ang_vel_body_enu)
        # quad_xyz_ang_vel_body_ned = quad_xyz_ang_vel_body_enu
        # quad_xyz_ang_vel_body_ned[1:] *= -1
        # quad_xyz_ang_vel_body_sp = u[:3].squeeze() + quad_xyz_ang_vel_body_ned

        xyz_torques_enu = u[:3].squeeze()

        xyz_torques_ned = self.enu2ned(xyz_torques_enu)
        # xyz_torques_ned = xyz_torques_enu
        # xyz_torques_ned[1:] *= -1
        
        quad_xyz_ang_vel_body_sp_ned = xyz_torques_ned + quad_xyz_ang_vel_body_ned

        quad_xyz_ang_vel_body_sp_enu = self.ned2enu(quad_xyz_ang_vel_body_sp_ned)        
        # quad_xyz_ang_vel_body_sp_enu = quad_xyz_ang_vel_body_sp_ned 
        # quad_xyz_ang_vel_body_sp_enu[1:] *= -1

        # print(f"===========================================================================")
        # print(f"{x.T=}")
        # print(f"{u.T=}")
        # print(f"{quad_xyz_ang_vel_body.T=}")
        # print(f"{quad_xyz_ang_vel_body_sp=}")
        # rospy.loginfo_throttle(1.0, f"{u.T=}")
        rospy.loginfo(f"{xyz_torques_ned.T=}")
        # print(quad_zyx_ang_vel_body_sp.shape, (u[:3].squeeze()).shape, quad_zyx_ang_vel_body.shape)
        self.att_setpoint.header.stamp = rospy.Time.now()
        self.att_setpoint.body_rate.x = float(quad_xyz_ang_vel_body_sp_enu[0])
        self.att_setpoint.body_rate.y = float(quad_xyz_ang_vel_body_sp_enu[1])
        self.att_setpoint.body_rate.z = float(quad_xyz_ang_vel_body_sp_enu[2])
    ...
    def set_rate_gains(self, gains_dict):
        rospy.wait_for_service('/mavros/param/set')
        try:
            param_set = rospy.ServiceProxy('/mavros/param/set', ParamSet)    
            for gain_name, value in gains_dict.items():
                resp = param_set(gain_name, ParamValue(real=value))
                if resp.success:
                    rospy.loginfo("Set rate gain: {} to {} successfully!".format(gain_name, value))
                else:
                    rospy.loginfo("Failed to set rate gain: {} to {}!".format(gain_name, value))
                rospy.sleep(0.25)
        except rospy.ServiceException as e:
            rospy.logerr("Failed to set rate gains: %s", e)
    ...
    def run(self, duration):
        ...
        ##### Takeoff Sequence #####
        self.set_rate_gains(self.gains_dict_px4_sim)
        # self.set_rate_gains(self.gains_dict_lqr)
