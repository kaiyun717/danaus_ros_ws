...
class DataCollectionRateAccel:
    ...
    def _change_goal(self):
        if self.track_type == "xyz":
            ...
            random_gamma = np.random.uniform(-np.pi/2.5, np.pi/2.5, 1)
            random_beta = np.random.uniform(-np.pi/2.5, np.pi/2.5, 1)
            random_alpha = np.random.uniform(-np.pi/2.5, np.pi/2.5, 1)
